...
|                                                  |                                       |
| ------------------------------------------------ | ------------------------------------- |
| [fun with content](./projects/fun-with-content/) | Content Projection in Angular |
...
* We saw that the directive may use dependency injection to get a reference to the parent component that received the content
  * We saw that we can use the `optional` property of the `inject` function to mark the parameter as optional, so that it does not crash if the directive is not used under the parent component
...
* We saw that we can travel **down** the hierarchy by using the `contentChild` function.
  * The syntax is `readonly child = contentChild(type)` which returns a `signal<type>`
* We saw that the member is not populated during construction time but it can be used inside the effects
